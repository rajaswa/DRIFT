...
def crawl_arxiv(arxiv_query, max_results, json_save_path):
    search = arxiv.Search(query=arxiv_query, max_results=max_results,sort_by=arxiv.SortCriterion.Relevance, sort_order=arxiv.SortOrder.Descending)
    ...
    for result in tqdm(search.get()):
        try:
            ...
            papers[year].append({"url": str(result.entry_id), "date":str(result.published), "title":str(result.title), "authors":[str(author) for author in result.authors], "abstract":str(result.summary), "journal ref":str(result.journal_ref), "category":str(result.primary_category)})
        except:
            continue
