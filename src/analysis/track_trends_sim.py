...
def compute_similarity_matrix_years(model_paths, keyword, top_k=5):
    ...
    for model_path in model_paths[1:]: 
        second_year_vocab, second_year_embs = get_word_embeddings(model_path, [], all_model_vectors=True, return_words=True)
        ...
        sim_dict = {**sim_dict,**{word +"_" + model_path:sim_vec[i] for i,word in enumerate(second_year_vocab)}}
