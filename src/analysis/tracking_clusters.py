...
def k_means_clustering(keywords, model_path, save_path, k_opt=None, k_max=10):
	...
	if k_opt is None:
		...
		for n_clusters in tqdm(range(2, k_max+1)):
			kmeans = KMeansClusterer(n_clusters, distance=nltk.cluster.util.cosine_distance, repeats=25)
	...
	for i in range(len(keywords)):
		df = df.append({"x":X_red[i][0], "y":X_red[i][1], "label":labels[i]}, ignore_index=True)

	facet = sns.lmplot(data=df, x='x', y='y', hue='label', 
                   fit_reg=False, legend=True, legend_out=True)
	facet.savefig("sample.png")
