...
def lda_basic(list_of_list_of_tokens, num_topics):
	...
	lda_model = models.LdaModel(corpus, num_topics=num_topics, id2word=dictionary_LDA, passes=4, alpha="auto", eta="auto", random_state=42)
...
def extract_topics_lda(text_file_paths, num_topics=0, num_words=10):
	...
	if num_topics == 0:
		coherence_scores = []
		range_of_topics = list(range(10,31))
		for num_topics_in_lst in range_of_topics:
			_, lda_model, coherence = lda_basic(list_of_list_of_tokens, num_topics_in_lst)
	...
	return year_wise_topics, lda_model.show_topics(formatted=True, num_topics=num_topics, num_words=num_words)
