...
def find_most_similar_words(
    ...
    word_vectors = np.array([year_model.wv[word] if word in year_model.wv.vocab else unk_emb for word in words])
    ...
    most_sim_words = [[compass_words[top_sims[i][j]] for j in range(top_sims[i].shape[0])] for i in range(top_sims.shape[0])]
    ...
    for idx, word in enumerate(words):
        ...
        for sim_word in most_sim_words[idx]:
            ...
            sim_dict[word]["sim_info"][sim_word]["sim_score"] = sim_matrix[words.index(word)][compass_words.index(sim_word)]
    return sim_dict


def find_most_drifted_words(words, year_model_paths, compass_model_path, top_k_for_sim=10, top_most_drifted_k=5):
    
    sim_dicts = [find_most_similar_words(words, year_model_path, compass_model_path, top_k_for_sim) for  year_model_path in year_model_paths]
    scores = {}
    for word in sim_dicts[0]:
        ...
        for sim_word in common_sim_words:
            scores[word] += abs(year_last_info[sim_word]["sim_score"] - year_1_info[sim_word]["sim_score"])

        scores[word] += len(year_1_sim_words + year_last_sim_words) - 2*len(common_sim_words)

    sorted_scores = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:top_most_drifted_k]

    words_for_plotting = []
    embs_for_plotting = []
    for idx, year_sim_dict in enumerate(sim_dicts):
        for word in sorted_scores:
            words_for_plotting.append(word + str(idx))
            ...
            for sim_word in year_sim_dict[word]["sim_info"]:
                if sim_word not in words_for_plotting:
                    ...
                    embs_for_plotting.append(year_sim_dict[word]["sim_info"][sim_word]["emb"])
